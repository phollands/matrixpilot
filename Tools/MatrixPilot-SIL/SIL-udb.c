...
void udb_init(void) {
	...
	gpsSocket = UDBSocket_init(
			(SILSIM_GPS_RUN_AS_SERVER) ? UDBSocketUDPServer : UDBSocketUDPClient,
					SILSIM_GPS_PORT, SILSIM_GPS_HOST, NULL, 0);
	telemetrySocket = UDBSocket_init(
			(SILSIM_TELEMETRY_RUN_AS_SERVER) ? UDBSocketUDPServer : UDBSocketUDPClient,
					SILSIM_TELEMETRY_PORT, (char*)SILSIM_TELEMETRY_HOST, NULL, 0);

	if (strlen(SILSIM_SERIAL_RC_INPUT_DEVICE) > 0) {
		printf("serial RC input active\n");
		serialSocket = UDBSocket_init(UDBSocketSerial, 0, NULL,
				(char*)SILSIM_SERIAL_RC_INPUT_DEVICE, SILSIM_SERIAL_RC_INPUT_BAUD);
	}
}

// loop interval in msec
#define UDB_STEP_TIME (1000 / HEARTBEAT_HZ)
...
//				get_current_time(&curseconds, &curmsec);
//				float curTime = (float) (curseconds - startseconds) + (float)curmsec/1000.0;
//				printf("udb_heartbeat_counter: %i, current_time: %g\n", udb_heartbeat_counter, curTime);
//				printf("xaccel: %i, yaccel: %i, zaccel: %i, xrate: %i, yrate: %i, zrate: %i\n",
//						XACCEL_VALUE, YACCEL_VALUE, ZACCEL_VALUE, XRATE_VALUE, YRATE_VALUE, ZRATE_VALUE);
//			}
...
void udb_a2d_record_offsets(void) {
	...
	printf("offsets: xaccel: %i, yaccel: %i, zaccel: %i, xrate: %i, yrate: %i, zrate: %i\n",
			UDB_XACCEL.offset, UDB_YACCEL.offset, UDB_ZACCEL.offset, udb_xrate.offset, udb_yrate.offset, udb_zrate.offset);
}
...
uint16_t get_current_milliseconds() {
	...
	// returns number of msec in range [0, 1000) (rolls over at 1000)
	struct timeval tv;
	struct timezone tz;
...
void get_current_time(uint32_t *seconds, uint16_t *msec) {
	// *nix / mac implementation
	// returns number of msec in range [0, 1000) (rolls over at 1000)
	struct timeval tv;
	struct timezone tz;
	...
	if (numServos && bytesRead >= headerBytes + numServos * 2 + 2) {
		...
		if (CK_A == buffer[headerBytes + numServos * 2]
			...
			for (i = 1; i <= numServos; i++) {
				udb_pwIn[i] = (uint16_t) (buffer[headerBytes + (i - 1) * 2])
						* 256 + buffer[headerBytes + (i - 1) * 2 + 1];
			}
		}
	}
}
...
void I2C_doneReadMagData(void) {
	...
	if (magMessage == 7) {
		udb_magFieldBody[0] = MAG_X_SIGN((__builtin_mulsu(
				(magFieldRaw[MAG_X_AXIS]), magGain[MAG_X_AXIS])) >> 14)
				- (udb_magOffset[0] >> 1);
		udb_magFieldBody[1] = MAG_Y_SIGN((__builtin_mulsu(
				(magFieldRaw[MAG_Y_AXIS]), magGain[MAG_Y_AXIS])) >> 14)
				- (udb_magOffset[1] >> 1);
		udb_magFieldBody[2] = MAG_Z_SIGN((__builtin_mulsu(
				(magFieldRaw[MAG_Z_AXIS]), magGain[MAG_Z_AXIS])) >> 14)
				- (udb_magOffset[2] >> 1);
