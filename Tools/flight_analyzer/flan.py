...
def convert_meters_north_to_lat(meters):
    """Converts relative meters North to Latitude  times 10 to the power 7
    (which is the native scale of Latitude returned from an EM606A GPS)"""
    return(90 * meters)
...
def write_GE_accel_vectors(log_book,flight_origin,filename, flight_clock):
    ...
    print >> filename, "</name>"
    counter = 0
    print >> filename, "<description>X accelerometer vectors plotted in the plane's body reference</description>"
    for entry in log_book.entries :
        counter += 1 
        #when = flight_clock.xml_time # Get the next time in XML format e.g.2007-01-14T21:05:02Z
        when = flight_clock.convert(entry.tm, log_book)
      ...
      <Location>
        ...
        print >> filename, """</latitude>
        <altitude>""",
        print >> filename, flight_origin.move_alt(entry.alt[primary_locator] + 0.5),
        print >> filename, """</altitude>
      </Location>
      <Orientation>
        <heading>""",
        ...
        print >> filename, """</heading>
        <tilt>""",
        # Using roll here on purpose instead of pitch
...
def write_flight_vectors(log_book,origin, filename, flight_clock,gps_delay) :
    ...
    for entry in log_book.entries :
      ...
      <Orientation>
        ...
        # the fuselage length of block_plane.dae is about 125 units in the y direction
        print >> filename, """</roll>
      </Orientation>
      <Scale>
        <x>2</x>
